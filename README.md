...
```
SharpADIDNS.exe enum --zone redteamnotes.local --domain-dn DC=redteamnotes,DC=local --server dc01.redteamnotes.local
```

Read one record and decode all blobs on it:

```
SharpADIDNS.exe query --zone redteamnotes.local --name fileserver --domain-dn DC=redteamnotes,DC=local
```
...
```
SharpADIDNS.exe add --zone redteamnotes.local --name web --type AAAA --data fe80::1 --domain-dn DC=redteamnotes,DC=local --server dc01.redteamnotes.local --username redteamnotes\alice --password "P@ss" --ldaps
```
